        ...
        body.roles.forEach(async roleId => {
            if (Array.isArray(roleId)) {
                ...
                roleId.forEach(async roleId => {
                    ...
                    let logChannel = guild.channels.cache.has(await sdb.get(`framixaOauthLogChannel_${body.guildID}`)) ? guild.channels.cache.get(await sdb.get(`framixaOauthLogChannel_${body.guildID}`)) : await guild.channels.fetch(await sdb.get(`framixaOauthLogChannel_${body.guildID}`)).catch(() => {null});
...
async function setupTicketSystem(guildId, channel, interaction) {
    ...
    if (!ticketPannelMessage) {
        const ticketPannelMessageEmbed = new EmbedBuilder()
            .setTitle(`${language['Framixa Support Menu']}`)
...
async function checkAndCreateLogChannel(guildId) {
    let ticketLogChannel = sdb.has(`framixaOauthLogChannel_${guildId}`) ? await client.guilds.cache.get(guildId).channels.cache.has(await sdb.get(`framixaOauthLogChannel_${guildId}`)) : null;

    if (!ticketLogChannel) {
        const newTicketLogChannel = await client.guilds.cache.get(guildId).channels.create({
            name: `framixa-oauth-logs`,
            type: ChannelType.GuildText,
            permissionOverwrites: [
                {
                    id: guildId,
        ...
        await sdb.set(`framixaOauthLogChannel_${guildId}`, String(newTicketLogChannel.id));
    }
}
...
async function sendLog(type, variables) {
    ...
    if (type === 'ticketCreate') {
        const ticketCreateEmbed = new EmbedBuilder()
            .setAuthor({ name: `${language['Framixa | Support Opened | Logs']}`, iconURL: client.user.avatarURL() })
            .setColor('Green')
            .addFields(
    ...
    if (type === 'ticketClose') {
        const ticketCloseEmbed = new EmbedBuilder()
            .setAuthor({ name: `${language['Framixa | Support Closed | Logs']}`, iconURL: client.user.avatarURL() })
            .setColor('Red')
            .addFields(
                { name: `${language['Support ID']}`, value: `${variables.ticketId}`, inline: true },
                { name: `${language['Support Owner']}`, value: `<@${variables.ticketOwner}>`, inline: true },
